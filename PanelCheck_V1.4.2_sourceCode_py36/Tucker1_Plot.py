...
def Tucker1Plotter(
    ...
    if itemID[0] not in collectedActiveItemsList:
        dlg = wx.MessageDialog(
            None,
            'The assessor or attribute is not active in CheckBox',
            'Error Message',
        ...
        if numberOfAttributes < 1:
            dlg = wx.MessageDialog(
                None,
                'There must be a minimum of 1 attribute.',
                'Error Message',
                wx.OK | wx.ICON_INFORMATION)
            dlg.ShowModal()
            dlg.Destroy()
    ...
    # those specific attributes.
        if not calculated or PCAmodeSelected[0] == 1:
            ...
            if itemID[0] in activeAttributesList and itemID[0] not in newActiveAttributesList:
                message = itemID[0] + \
                    ' was left out of the analysis because for one or \nmore assessors the standard deviation is 0: \n'
    ...
    if itemID[0] in activeAttributesList:
        ...
        if subplot:  # is subplot
            plot_data.ax = plot_data.fig.add_subplot(
                num_subplot[0], num_subplot[1], num_subplot[2], aspect=plot_data.aspect)
        else:
                ...
                if selection[1] == 0:
                    ...
                    for position in range(numberOfAssessors):
                        newPosition = position * \
                            len(activeAttributesList) + specificPositionAttribute
...
def Tucker1AssOverviewPlotter(
    ...
    return OverviewPlotter(
        ...
        special_selection=pca_mode_select,
        abspath=abspath)


def Tucker1AttOverviewPlotter(
        s_data,
        plot_data,
        pca_mode_select,
        abspath=None):
